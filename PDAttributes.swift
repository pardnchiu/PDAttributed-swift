...
var defaultFont: UIFont {
	get { return UIFont.systemFont(ofSize: 15, weight: .regular) }
};
...
extension UILabel {
	...
	var fontSize	: CGFloat { get { return self.font.pointSize } };
...
public extension UITextField {
	...
	func attr(name: String, size: CGFloat,_ target: __attr_target) -> UITextField {
		if (target == .text) {
			...
			self.attrPlh = setPlhValue(.font, UIFont(name: name, size: size) ?? defaultFont);
		};
		return self;
	};
	func attr(name: String,_ target: __attr_target) -> UITextField {
		if (target == .text) {
			...
			self.attrPlh = setPlhValue(.font, UIFont(name: name, size: self.fontSize) ?? defaultFont);
		};
		return self;
	};
	func attr(size: CGFloat,_ target: __attr_target) -> UITextField {
		if (target == .text) {
			...
			self.attrPlh = setPlhValue(.font, UIFont(name: self.fontName, size: size) ?? defaultFont);
		};
		return self;
	};
